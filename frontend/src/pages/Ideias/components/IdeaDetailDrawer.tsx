...
export function IdeaDetailDrawer({
  ...
  // 👇 1. O Apollo normal com a regra 'skip' resolve o Erro 400
  const { data, loading } = useQuery<{ getIdea: Idea }>(GET_IDEA, {
    variables: { ideaId },
    skip: !ideaId, // Se ideaId for null, o Apollo nem tenta fazer a requisição!
  })
    ...
    <Drawer open={open} onOpenChange={onOpenChange} direction="right">
      <DrawerContentRight className="flex flex-col rounded-l-2xl">
        <div className="shrink-0 p-6 bg-slate-100 rounded-l-2xl">
          ...
          {/* 👇 2. Tiramos a descrição de dentro do if. Fim do aviso de acessibilidade! */}
          <DrawerDescription className="text-sm text-muted-foreground leading-relaxed mt-2">
            {idea?.description || "Carregando detalhes..."}
          </DrawerDescription>
          {/* {idea && (
